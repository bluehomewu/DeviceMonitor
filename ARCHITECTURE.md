...
### 當前裝置識別：用 thisDeviceId，不要用 Build.MODEL

`AppModule.thisDeviceId` 是 ANDROID_ID，每台實體裝置唯一，是識別「本機」的正確依據。

`Build.MODEL` 是硬體型號名稱（例如 `Pixel 8 Pro`），同款裝置會撞名。若夥伴分享了一台與本機同型號的裝置，`deviceStateHolder.devices` 裡會有兩筆 `deviceName == Build.MODEL` 的記錄，用 `find { it.deviceName == Build.MODEL }` 可能找到夥伴的裝置。

```kotlin
// ✅ 正確
deviceStateHolder.devices.value.find { it.deviceId == AppModule.thisDeviceId }

// ❌ 同款機型時會找到夥伴裝置
deviceStateHolder.devices.value.find { it.deviceName == Build.MODEL }
```
...
| `values-zh-rHK/` | 繁體中文（香港）— 使用「網絡」、「電訊商」等港式慣用詞 |
| `values-zh-rCN/` | 簡體中文 |

新增或修改 `values/strings.xml` 中的字串後，四個語系都要同步更新，否則 Android 會回落至英文預設值。
