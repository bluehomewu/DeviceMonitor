...
### 1. 更新版本號（`app/build.gradle.kts`）

| 變更類型 | versionName 規則 | versionCode |
|---|---|---|
| Bug fix | PATCH +1（`1.1.0 → 1.1.1`） | +1 |
| 新功能 | MINOR +1（`1.1.x → 1.2.0`，PATCH 歸零） | +1 |
| 重大 breaking change | MAJOR +1（`1.x.x → 2.0.0`） | +1 |
